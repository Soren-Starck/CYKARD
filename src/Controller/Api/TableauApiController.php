...
class TableauApiController extends GeneriqueController
{
    ...
    {
        $result = $this->tableauService->modifyTableau($request, $id);
    ...
    {
        $result = $this->tableauService->showTableau($request, $id);
        if (isset($result['error'])) return $this->json(['error' => $result['error']], $result['status']);
        return $this->json($result, 200);
    }
    ...
    {
        $result = $this->tableauService->deleteTableau($request, $id);
        if (isset($result['error'])) return $this->json(['error' => $result['error']], $result['status']);
    ...
    {
        $result = $this->tableauService->createTableau($request);
        if (isset($result['error'])) return $this->json(['error' => $result['error']], $result['status']);
    ...
    {
        $result = $this->tableauService->joinTableau($request, $codetableau);
        if (isset($result['error'])) return $this->json(['error' => $result['error']], $result['status']);
        return $this->json($result, 201);
    }
}